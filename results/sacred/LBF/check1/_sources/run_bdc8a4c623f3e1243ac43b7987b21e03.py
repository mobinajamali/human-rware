...
def run_sequential(args, logger):
    ...
    agent_helping_mode = args.agent_help_mode
    agent_help_flag = False

    runner = r_REGISTRY[args.runner](
        args=args,
        logger=logger,
        agent_helping_mode=agent_helping_mode,
    ...
    while runner.t_env <= args.t_max:
        ...
        if buffer.can_sample(args.batch_size):
            ...
            max_exp_t = expert_episode_sample.max_t_filled()
            
            # print("CHECK THIS HERE", max_ep_t, max_exp_t)
            ...
            learner.train(
                ...
                expertActions,
                trainingActions,
                expert_episode_sample,
                timesteps, 
                terminated_info
            )            
            
            #Switch off the agent_help_flag again
            agent_help_flag = False
        ...
        if args.save_model and (
            ...
            # token = args.unique_token.split(' ')[0]
            token =  re.sub(r':', '_', args.unique_token)
            save_path = os.path.join(
                args.local_results_path, "models", token, #str(runner.t_env)
            )
            ...
            if args.use_wandb and args.wandb_save_model:
                wandb_save_dir = os.path.join(
                    logger.wandb.dir, "models", token, #str(runner.t_env)
                )
